...
class PlayState extends MusicBeatState
{
	...
	public static var theFunne:Bool = false;
	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'stronger':
				dialogue = CoolUtil.coolTextFile(Paths.txt('stronger/strongerDialogue'));
			case 'megalomaniac':
				dialogue = CoolUtil.coolTextFile(Paths.txt('megalomaniac/megalomaniacDialogue'));
			case 'reality-check':
				dialogue = CoolUtil.coolTextFile(Paths.txt('reality-check/reality-checkDialogue'));
			case 'bone-battle':
				dialogue = CoolUtil.coolTextFile(Paths.txt('bone-battle/bone-battleDialogue'));
			case 'darker':
				dialogue = CoolUtil.coolTextFile(Paths.txt('darker/darkerDialogue'));
			case 'nightmare':
				dialogue = CoolUtil.coolTextFile(Paths.txt('nightmare/nightmareDialogue'));
			case 'strikes':
				dialogue = CoolUtil.coolTextFile(Paths.txt('strikes/strikesDialogue'));
		}
			...
			|| SONG.song.toLowerCase() == 'reality-check')
		{
			...
			var bg:FlxSprite = new FlxSprite(-400, -150).loadGraphic(Paths.image('Sansbg'));
		...
		else if (SONG.song.toLowerCase() == 'darker')
		{
			defaultCamZoom = 0.9;
			curStage = 'void';
			var bg:FlxSprite = new FlxSprite(-1400, -560).loadGraphic(Paths.image('Gasterbg'));
			// bg.setGraphicSize(Std.int(bg.width * 1.1));
		...
		else if (SONG.song.toLowerCase() == 'bone-battle')
		{
			defaultCamZoom = 0.9;
			curStage = 'terror';
			var bg:FlxSprite = new FlxSprite(-600, -100).loadGraphic(Paths.image('papBgResized'));
			// bg.setGraphicSize(Std.int(bg.width * 1.1));
			bg.antialiasing = true;
			bg.scrollFactor.set(0.9, 0.9);
			bg.active = false;
		...
		else if (SONG.song.toLowerCase() == 'strikes')
		{
			defaultCamZoom = 0.9;
			curStage = 'nothingness';
			var bg:FlxSprite = new FlxSprite(-600, -100).loadGraphic(Paths.image('Charabg'));
			// bg.setGraphicSize(Std.int(bg.width * 1.1));
			bg.antialiasing = true;
			bg.scrollFactor.set(0.9, 0.9);
			bg.active = false;
		...
		else if (SONG.song.toLowerCase() == 'nightmare')
		{
			flower = new FlxSprite(0, 500).loadGraphic(Paths.image('Flowerr'));
			add(flower);
		...
		kadeEngineWatermark = new FlxText(FlxG.width * 0.03, FlxG.height * 0.94, 0,
			SONG.song
			+ " "
		...
		if (isStoryMode)
		{
			switch (curSong.toLowerCase())
			{
				...
				case 'thorns':
					schoolIntro(doof);
				case 'stronger' | 'megalomaniac' | 'reality-check' | 'bone-battle' | 'darker' | 'nightmare' | 'strikes':
					schoolIntro(doof);
				default:
					startCountdown();
			}
	...
	override public function update(elapsed:Float)
	{
		...
		if (curSong == 'Nightmare')
		{
			...
			{
				case -15:
					blackMama = new FlxSprite(-FlxG.width * FlxG.camera.zoom,
						-FlxG.height * FlxG.camera.zoom).makeGraphic(FlxG.width * 3, FlxG.height * 3, FlxColor.BLACK);
				case -10:
					blackMama.scrollFactor.set();
					add(blackMama);
					camHUD.visible = false;
					dad.visible = true;
					remove(flower);
	...
	function endSong():Void
	{
		...
		if (isStoryMode)
		{
			...
			if (storyPlaylist.length <= 0)
			{
				...
				if (SONG.song.toLowerCase() == 'strikes')
					FlxG.save.data.secretWeek = true;

				if (storyDifficulty == 2 && SONG.song.toLowerCase() == 'reality-check' && !FlxG.save.data.secretWeek)
